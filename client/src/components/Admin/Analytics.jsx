// src/components/Admin/Analytics.jsx — Full Analytics Dashboard with Real API Data
...
const FULFILL_COLORS = { Delivered: '#10b981', Shipped: '#8b5cf6', Processing: '#3b82f6', Confirmed: '#6b7280', Returned: '#ef4444' };
...
const TTP = {
  contentStyle: { background: '#1e1b18', border: '1px solid #3a3530', borderRadius: '10px', padding: '10px 14px' },
...
// Fixed-height wrapper so Recharts can measure properly
...
const StatCard = ({ icon, label, value, trend, color = '#D4AF37', sub, delay = 0 }) => (
  <motion.div className="an-stat-card" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
    transition={{ delay }} style={{ '--accent': color }}>
    <div className="an-stat-icon" style={{ background: `${color}22`, color }}>{icon}</div>
...
const TabLoader = () => (
  <div className="an-loading-screen" style={{ height: '40vh' }}>
    <div className="an-spinner" /><p>Loading…</p>
  </div>
);
...
const Analytics = () => {
  ...
  /* fetch overview whenever timeframe changes */
  ...
  useEffect(() => {
    const load = async () => {
      try {
        if (activeTab === 'abandoned' && !abandoned) {
          const r = await getAbandonedCartsReport(); if (r?.data?.success) setAbandoned(r.data.data);
        }
        if (activeTab === 'payments' && !payments) {
          const r = await getPaymentsReport(); if (r?.data?.success) setPayments(r.data.data);
        }
        if (activeTab === 'customers' && !customers) {
          const r = await getCustomersReport(); if (r?.data?.success) setCustomers(r.data.data);
        }
        if (activeTab === 'inventory' && !inventory) {
          const r = await getInventoryReport(); if (r?.data?.success) setInventory(r.data.data);
        }
        if (activeTab === 'coupons' && !coupons) {
          const r = await getCouponsReport(); if (r?.data?.success) setCoupons(r.data.data);
        }
  ...
  const exportOrders = async () => {
    try { setExporting(true); const r = await exportOrdersCSV(); downloadBlob(r.data, `orders-${Date.now()}.csv`); }
    catch (e) { console.error(e); } finally { setExporting(false); }
  };
  const exportCusts = async () => {
    try { setExporting(true); const r = await exportCustomersCSV(); downloadBlob(r.data, `customers-${Date.now()}.csv`); }
    catch (e) { console.error(e); } finally { setExporting(false); }
  };
  ...
  const salesData = (a.salesData || []).map(d => ({
    date: d._id ? new Date(d._id).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) : '',
    revenue: Number(d.total || 0),
    orders: Number(d.orders || 0),
  }));

  // Filter out rows where both are 0 so the chart isn't a flat line when there's no data
  ...
  const trendLabel = (trend) => trend !== undefined
  ...
  if (loading && !overview) return (
    <div className="an-loading-screen"><div className="an-spinner" /><p>Loading analytics…</p></div>
  );
  ...
  return (
    <div className="analytics-dashboard">
      ...
      <div className="an-header">
        ...
        <div className="an-header-controls">
          <div className="an-timeframe-pills">
            {TIMEFRAMES.map(tf => (
              <button key={tf.value} className={`an-tf-pill${timeframe === tf.value ? ' active' : ''}`}
                onClick={() => setTimeframe(tf.value)}>{tf.label}</button>
            ))}
          </div>
          <div className="an-export-btns">
            <button className="an-export-btn" onClick={exportOrders} disabled={exporting}><FaDownload /> Orders CSV</button>
            <button className="an-export-btn an-export-btn--alt" onClick={exportCusts} disabled={exporting}><FaDownload /> Customers CSV</button>
          </div>
          <button className="an-refresh-btn" onClick={loadOverview} disabled={loading}>
      ...
      <div className="an-tab-nav">
        {TABS.map(t => (
          <button key={t.id} className={`an-tab${activeTab === t.id ? ' active' : ''}`}
      ...
      <AnimatePresence mode="wait">
        ...
        {/* ══ OVERVIEW ══ */}
        {activeTab === 'overview' && (
          <motion.div key="ov" className="an-section" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
            ...
            <div className="an-charts-row">
              <ChartCard wide title="Revenue Trajectory"
                ...
                {hasSalesData ? (
                  ...
                  <Empty
                    icon="📈"
                    msg={lastOrderDate
                      ? `No sales data for this period. Your last order was on ${lastOrderDate.toLocaleDateString()}. Try selecting "All Time" or "90 Days".`
                  ...
                  <ChartBox height={280}>
                    <PieChart>
                      <Pie data={fulData} cx="50%" cy="45%" innerRadius={65} outerRadius={95} paddingAngle={3} dataKey="value">
                        {fulData.map((e, i) => <Cell key={i} fill={FULFILL_COLORS[e.name] || COLORS[i]} />)}
                      </Pie>
        ...
        {/* ══ SALES ══ */}
        {activeTab === 'sales' && (
          <motion.div key="sa" className="an-section" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
            <div className="an-kpi-grid">
              ...
              <StatCard delay={0.18} icon={<FaFire />} color="#f59e0b" label="Peak Day" value={fmtKes(salesData.length > 0 ? Math.max(...salesData.map(d => d.revenue)) : 0)} />
            </div>
                  ...
                  <div className="an-category-legend">
                    {catData.map((cat, i) => (
                      <div key={i} className="an-cat-row">
                        ...
                        <div className="an-progress-bg"><div className="an-progress-fill" style={{ width: `${cat.value}%`, background: COLORS[i % COLORS.length] }} /></div>
        ...
        {/* ══ PRODUCTS ══ */}
        {activeTab === 'products' && (
          <motion.div key="pr" className="an-section" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
            <div className="an-kpi-grid">
            ...
            <ChartCard wide title="🏆 Top Products by Sales" subtitle="Ranked by units sold">
              {topProds.length > 0 ? (
                <div className="an-leaderboard">
                  {topProds.map((p, i) => (
                    <motion.div key={i} className={`an-leader-row${i === 0 ? ' gold' : i === 1 ? ' silver' : i === 2 ? ' bronze' : ''}`}
                      initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.06 }}>
                      <div className="an-leader-rank">{i === 0 ? '🥇' : i === 1 ? '🥈' : i === 2 ? '🥉' : <span className="an-rank-num">{i + 1}</span>}</div>
        ...
        {/* ══ CUSTOMERS ══ */}
        {activeTab === 'customers' && (
          <motion.div key="cu" className="an-section" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
            {!customers ? <TabLoader /> : (<>
              <div className="an-kpi-grid">
              ...
              {customers.topReturning?.length > 0 && (
                <ChartCard wide title="Top Returning Customers"
                  action={<button className="an-export-btn" onClick={exportCusts} disabled={exporting}><FaDownload /> CSV</button>}>
                  <div className="an-table-wrap">
                    <table className="an-table">
                      <thead><tr><th>#</th><th>Name</th><th>Email</th><th>Orders</th><th>Spent</th></tr></thead>
                      <tbody>{customers.topReturning.map((c, i) => (
                        <tr key={i}>
                          <td>{i + 1}</td>
                          <td><strong>{c.name}</strong></td>
                          <td className="an-muted">{c.email}</td>
                          <td><span className="an-badge" style={{ background: '#3b82f622', color: '#3b82f6', border: '1px solid #3b82f644' }}>{c.orders}</span></td>
        ...
        {/* ══ PAYMENTS ══ */}
        {activeTab === 'payments' && (
          <motion.div key="pa" className="an-section" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
            {!payments ? <TabLoader /> : (<>
              <div className="an-kpi-grid">
                ...
                <ChartCard wide title="Recent Refunds">
                  <div className="an-table-wrap">
                    <table className="an-table">
                      ...
                      <tbody>{payments.recentRefunds.map((r, i) => (
                        <tr key={i}>
        ...
        {/* ══ INVENTORY ══ */}
        {activeTab === 'inventory' && (
          <motion.div key="iv" className="an-section" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
            {!inventory ? <TabLoader /> : (<>
              <div className="an-kpi-grid">
              ...
              <div className="an-secondary-kpis">
                <div className="an-secondary-card" style={{ gridColumn: 'span 4' }}>
                  ...
                  <span className="an-sk-value" style={{ fontSize: '1.8rem' }}>{fmtKes(inventory.summary?.totalInventoryValue)}</span>
                </div>
              </div>
              ...
              <ChartCard wide title="Stock Levels" subtitle="Top 15 products by current stock">
                {inventory.stockList?.length > 0 ? (
                  <ChartBox height={Math.max(300, Math.min(inventory.stockList.length, 15) * 38 + 80)}>
                    <BarChart data={inventory.stockList.slice(0, 15)} layout="vertical"
                ...
                <ChartCard wide title="⚠️ Low Stock Alerts" subtitle="Products that need restocking">
                  <div className="an-table-wrap">
                    <table className="an-table">
                      ...
                      <tbody>{inventory.lowStockItems.map((p, i) => (
                        <tr key={i}>
        ...
        {/* ══ ABANDONED ══ */}
        {activeTab === 'abandoned' && (
          <motion.div key="ab" className="an-section" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
            {!abandoned ? <TabLoader /> : (<>
              <div className="an-kpi-grid">
              ...
              {abandoned.recentAbandoned?.length > 0 && (
                <ChartCard wide title="Recent Abandoned Orders">
                  <div className="an-table-wrap">
                    <table className="an-table">
                      <thead><tr><th>Order</th><th>Customer</th><th>Items</th><th>Value</th><th>Date</th></tr></thead>
                      <tbody>{abandoned.recentAbandoned.map((o, i) => (
                        <tr key={i}>
                          <td><code style={{ color: '#D4AF37' }}>#{o.orderNumber}</code></td>
                          <td><strong>{o.customerName}</strong><div className="an-muted">{o.email}</div></td>
                          <td>{o.items} items</td>
        ...
        {/* ══ COUPONS ══ */}
        {activeTab === 'coupons' && (
          <motion.div key="co" className="an-section" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
            {!coupons ? <TabLoader /> : (<>
              <div className="an-kpi-grid">
              ...
              {coupons.coupons?.length > 0 ? (
                <>
                  <ChartCard wide title="Coupon Performance" subtitle="Code usage &amp; revenue impact">
                    <div className="an-table-wrap">
                      <table className="an-table">
                        <thead><tr><th>Code</th><th>Uses</th><th>Discount Given</th><th>Revenue</th></tr></thead>
                        <tbody>{coupons.coupons.map((c, i) => (
                          <tr key={i}>
